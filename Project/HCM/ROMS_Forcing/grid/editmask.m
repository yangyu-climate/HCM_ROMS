...
if (nargin < 2 | isempty(coast_file)),
  coast_file='coastline_i_mask.mat';
end,
...
% grid indices.
   XLON = ncread(grid_file,'lon_rho');
   rlon_max = nanmax(nanmax(XLON));
   if rlon_max > 180
     IF_lon_change_sign=1;
   else
     IF_lon_change_sign=0;
   end
   ...
   elseif (exist('lon') & exist('lat')),
     [C]=ijcoast(grid_file,coast_file,IF_lon_change_sign);
     xcst=C.Icst;
     ycst=C.Jcst;
     clear C lat lon;
   else,
