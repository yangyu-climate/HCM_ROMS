...
#ifdef UV_COR_NT
      real(r8), intent(out) :: e(LBi:UBi,LBj:UBj)
#endif
...
#ifdef UV_COR_NT
          e(i,j)=val1*COS(latr(i,j)*deg2rad)
#endif
        END DO
      END DO
#elif defined WEDDELL
...
#ifdef UV_COR_NT
          e(i,j)=2.0_r8*7.2E-05_r8*                                     &
     &           COS((-79.0_r8+REAL(i-1,r8)*val1)*deg2rad)
#endif
        END DO
      END DO
#else
...
!
     ...
     &                  LBi, UBi, LBj, UBj, e)
      IF (DOMAIN(ng)%NorthEast_Corner(tile)) THEN
        WRITE (stdout,10) 'NT Coriolis parameter at RHO-points: e',     &
     &                     ng, Stats(14)%min, Stats(14)%max
      END IF
!
!  Exchange boundary data.
