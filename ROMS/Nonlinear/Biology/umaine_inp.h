...
# endif
            ...
            CASE ('Dout(iPPro3)')
              IF (iDbio3(iPPro3).eq.0) THEN
                IF (Master) WRITE (out,40) 'iDbio3(iPPro3)'
                exit_flag=5
                RETURN
              END IF
              Npts=load_l(Nval, Cval, Ngrids, Lbio)
...
!
     ...
     &            'Empirical Fe:C coefficient (1/M-C).'
            WRITE (out,100) S1_FeC(ng), 'S1_FeC',                        &
     &            'Small P Fe:C at F=0.5 (muM-Fe/M-C).'
            WRITE (out,100) S2_FeC(ng), 'S2_FeC',                        &
     &            'Diatoms Fe:C at F=0.5 (muM-Fe/M-C).'
            WRITE (out,100) S3_FeC(ng), 'S3_FeC',                        &
