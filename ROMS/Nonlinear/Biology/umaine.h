...
!
      ...
      real(r8) :: UFeS1
      real(r8) :: FeC2FeN, FeN2FeC, N2C, N2CH, N2Fe
      real(r8) :: FeN2FeCS1,FeN2FeCS2,FeN2FeCS3
...
#ifdef IRON_LIMIT
!
!  Set N:C and N:CH conversion ratio.
!  Set Fe:N and Fe:C conversion ratio and its inverse.
!
        N2C    = 106.0_r8/16.0_r8
        N2CH   =  0.05_r8*12.0_r8*N2C
        N2Fe   =  k_FeC*N2C
...
!  Only nudge if we have too little iron
	    IF (Bio(i,k,iFeD_) .le. Fe_clm) THEN
     ...
     &                       dt(ng)*Fndgcf*(Fe_clm-Bio(i,k,iFeD_))
        ...
        DO k=1,N(ng)
          DO i=Istr,Iend
!  Nudge if we have too large iron error
	    IF ( ( Bio(i,k,iS1_Fe) .ge. 500.0_r8*Bio(i,k,iFeD_) )           &
     &     .or.  ( Bio(i,k,iS1CH)  .ge. 15.0_r8                   ) ) THEN
              Bio(i,k,iS1_N) =Bio(i,k,iS1_N)-                               &
     &                 3.0_r8*dt(ng)*Fndgcf*(Bio(i,k,iS1_N)-0.1_r8)
              Bio(i,k,iS1_C) =Bio(i,k,iS1_C)-                               &
     &                 3.0_r8*dt(ng)*Fndgcf*(Bio(i,k,iS1_C)-0.1_r8*N2C)
              Bio(i,k,iS1CH) =Bio(i,k,iS1CH)-                               &
     &                 3.0_r8*dt(ng)*Fndgcf*(Bio(i,k,iS1CH)-0.1_r8*N2CH)
              Bio(i,k,iS1_Fe)=Bio(i,k,iS1_Fe)-                              &
     &                 3.0_r8*dt(ng)*Fndgcf*(Bio(i,k,iS1_Fe)-0.1_r8*N2Fe)
	    ENDIF
	    IF ( ( Bio(i,k,iS2_Fe) .ge. 500.0_r8*Bio(i,k,iFeD_) )           &
     &     .or.  ( Bio(i,k,iS2CH)  .ge. 15.0_r8                   ) ) THEN
              Bio(i,k,iS2_N) =Bio(i,k,iS2_N)-                               &
     &                 3.0_r8*dt(ng)*Fndgcf*(Bio(i,k,iS2_N)-0.1_r8)
              Bio(i,k,iS2_C) =Bio(i,k,iS2_C)-                               &
     &                 3.0_r8*dt(ng)*Fndgcf*(Bio(i,k,iS2_C)-0.1_r8*N2C)
              Bio(i,k,iS2CH) =Bio(i,k,iS2CH)-                               &
     &                 3.0_r8*dt(ng)*Fndgcf*(Bio(i,k,iS2CH)-0.1_r8*N2CH)
              Bio(i,k,iS2_Fe)=Bio(i,k,iS2_Fe)-                              &
     &                 3.0_r8*dt(ng)*Fndgcf*(Bio(i,k,iS2_Fe)-0.1_r8*N2Fe)
	    ENDIF
	    IF ( ( Bio(i,k,iS3_Fe) .ge. 500.0_r8*Bio(i,k,iFeD_) )           &
     &     .or.  ( Bio(i,k,iS3CH)  .ge. 15.0_r8                   ) ) THEN
              Bio(i,k,iS3_N) =Bio(i,k,iS3_N)-                               &
     &                 3.0_r8*dt(ng)*Fndgcf*(Bio(i,k,iS3_N)-0.1_r8)
              Bio(i,k,iS3_C) =Bio(i,k,iS3_C)-                               &
     &                 3.0_r8*dt(ng)*Fndgcf*(Bio(i,k,iS3_C)-0.1_r8*N2C)
              Bio(i,k,iS3CH) =Bio(i,k,iS3CH)-                               &
     &                 3.0_r8*dt(ng)*Fndgcf*(Bio(i,k,iS3CH)-0.1_r8*N2CH)
              Bio(i,k,iS3_Fe)=Bio(i,k,iS3_Fe)-                              &
     &                 3.0_r8*dt(ng)*Fndgcf*(Bio(i,k,iS3_Fe)-0.1_r8*N2Fe)
	    ENDIF
	    IF (Bio(i,k,iFeD_) .ge. 3.0_r8*Fe_max) THEN
              Bio(i,k,iFeD_) =Bio(i,k,iFeD_)-                               &
     &                 3.0_r8*dt(ng)*Fndgcf*(Bio(i,k,iFeD_)-Fe_max)
	    ENDIF
          END DO
        END DO
#endif
        ...
        DO k=1,N(ng)
          DO i=Istr,Iend
!  Set zero if we have negative iron error
              Bio(i,k,iS1_N)=MAX(Bio(i,k,iS1_N),Minval)
              Bio(i,k,iS1_C)=MAX(Bio(i,k,iS1_C),Minval)
              Bio(i,k,iS1CH)=MAX(Bio(i,k,iS1CH),Minval)
              Bio(i,k,iS1_Fe)=MAX(Bio(i,k,iS1_Fe),Minval)

              Bio(i,k,iS2_N)=MAX(Bio(i,k,iS2_N),Minval)
              Bio(i,k,iS2_C)=MAX(Bio(i,k,iS2_C),Minval)
              Bio(i,k,iS2CH)=MAX(Bio(i,k,iS2CH),Minval)
              Bio(i,k,iS2_Fe)=MAX(Bio(i,k,iS2_Fe),Minval)

              Bio(i,k,iS3_N)=MAX(Bio(i,k,iS3_N),Minval)
              Bio(i,k,iS3_C)=MAX(Bio(i,k,iS3_C),Minval)
              Bio(i,k,iS3CH)=MAX(Bio(i,k,iS3CH),Minval)
              Bio(i,k,iS3_Fe)=MAX(Bio(i,k,iS3_Fe),Minval)

              Bio(i,k,iFeD_)=MAX(Bio(i,k,iFeD_),Minval)
          END DO
        END DO
        ...
        DO k=N(ng),1,-1
          DO i=Istr,Iend
             if(kdpar(i,k).ge.0.001_r8.and.kdpar(i,k).le.10.0_r8) then
                    ...
                    cff1=(AK1(ng)+(Bio(i,k,iS1CH)+Bio(i,k,iS2CH)+       &
     &               Bio(i,k,iS3CH))*AK2(ng))*HZ(i,j,k)
             endif
...
!             FeN2FeCS1=FeN2FeC
              FeN2FeCS1=MAX(Minval,Bio(i,k,iS1_N))                    &
     &                 /MAX(Minval,Bio(i,k,iS1_C))*1.0E3_r8

              FCratioS1=FNratioS1*FeN2FeCS1
...
! Current F:C ratio [umol-Fe/mol-C]
!             FeN2FeCS2=FeN2FeC
              FeN2FeCS2=MAX(Minval,Bio(i,k,iS2_N))                    &
     &                 /MAX(Minval,Bio(i,k,iS2_C))*1.0E3_r8

              FCratioS2=FNratioS2*FeN2FeCS2
...
! Current F:C ratio [umol-Fe/mol-C]
!           FeN2FeCS3=FeN2FeC
            FeN2FeCS3=MAX(Minval,Bio(i,k,iS3_N))                    &
     &               /MAX(Minval,Bio(i,k,iS3_C))*1.0E3_r8

            FCratioS3=FNratioS3*FeN2FeCS3
