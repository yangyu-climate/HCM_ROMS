...
#  endif
#  ifdef SSH_NUDGING
      real(r8), intent(inout) :: sshC(LBi:,LBj:)
#  endif
...
#  endif
#  ifdef SSH_NUDGING
      real(r8), intent(inout) :: sshC(LBi:UBi,LBj:UBj)
#  endif
...
# ifdef SSH_NUDGING
!
!  Apply SSH nudging on new free-surface.
!
      DO j=Jstr,Jend
        DO i=Istr,Iend
          zeta_new(i,j)=zeta_new(i,j)                                   &
     &      -dtfast(ng)*Tnudg_SSH(ng)*(zeta_new(i,j)-sshC(i,j))
        END DO
      END DO
# endif
...
# if defined WET_DRY && defined MASKING
     ...
     &                   (Dcrit(ng)-h(i,j))*(1.0_r8-rmask(i,j))
          IF (zeta(i,j,knew).NE.zeta(i,j,knew))  THEN
